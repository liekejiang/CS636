...
#         return lambda_rpn_class * K.sum( K.binary_crossentropy(y_pred[:, :, :, :], y_true[:, :, :, :num_anchors])) / K.sum(epsilon + y_true[:, :, :, :num_anchors])        
...
def class_loss_reg(num_classes):
    def class_loss_reg_fixed_num(y_true, y_pred):
        ...
        return lambda_cls_reg * K.sum(y_true[:,:,:4*num_classes] * (x_bool * (0.5 * x * x) + (1 - x_bool) * (x_abs - 0.5))) / K.sum(epsilon + y_true[:,:,:4*num_classes])
